...
export function TrainingPlanGenerator({ role }: TrainingPlanGeneratorProps) {
  ...
  const generatePlan = () => {
    ...
    // Get relevant missions for this role
...
${priorityMissions.map((m, i) => `${i + 1}. ${m.title}`).join('\n')}
...
`.trim();
  ...
  return (
    <div className="bg-card rounded-xl border border-border/50 p-6">
      ...
      <div className="mb-4">
        <label className="text-sm font-medium text-foreground mb-1.5 block">
          Top 3 Risks for Your Role (Optional)
        </label>
        <Textarea
          placeholder="Example:
1. Patch documentation gaps on legacy systems
2. Training records not centralized
3. ESP diagrams need updating"
          value={topRisks}
          onChange={(e) => setTopRisks(e.target.value)}
          className="min-h-[100px]"
        />
        <p className="text-xs text-muted-foreground mt-1">
          Describe specific compliance risks you're facing. These will be included in your generated plan.
        </p>
      </div>
